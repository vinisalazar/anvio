...
def migrate(db_path):
    ...
    contigs_db.create_table(contig_links_table_name, contig_links_table_structure, contig_links_table_types)

    contigs_db.remove_meta_key_value_pair('version')
    contigs_db.set_version(next_version)
    contigs_db.disconnect()

    run.info_single("The contigs database is now %s, and it has a shiny new links table in it" % \
